...
export const LoadVault = () => {
  ...
  return (
    ...
    >
      <View style={{ flex: 1 }}>
        {!isKeyboardVisible && (
          <View style={styles.logoContainer} testID="load_vault_logo">
        ...
        >
          <View style={styles.formContainer}>
            <View style={{ marginBottom: 20, alignItems: 'center', gap: 10 }}>
              <Text style={styles.title} testID="load_vault_title">{t`Load an existing Vault`}</Text>
              <Text
                style={styles.subtitle} testID="load_vault_subtitle"
              >{t`Open your vault with this code`}</Text>
            </View>

            <View style={{ width: '100%', gap: 15 }}>
              <InputPasswordPearPass
                testID="load_vault_invite_code_input"
            ...
            <View style={{ width: '100%', gap: 10, marginTop: 10 }}>
              {isLoading ? (
                <>
                  <ActivityIndicator
                    testID="load_vault_loading"
                    size="small"
                    color={colors.primary400.mode1}
                  />
                  <ButtonSecondary testID="load_vault_cancel_pairing_button" stretch onPress={cancelPairActiveVault}>
                    {t`Cancel Pairing`}
                  </ButtonSecondary>
                </>
              ) : (
                <>
                  ...
                  <ButtonSecondary
                    testID="load_vault_select_vaults_button"
                    stretch
                    onPress={() =>
                  ...
                  <Pressable
                    testID="load_vault_scan_qr_button"
                    accessibilityLabel="load_vault_scan_qr_button"
